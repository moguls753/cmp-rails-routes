        ...
        if return_val == 0 then
          ...
          if ok then
            ...
            for _, route in ipairs(routes) do
              local url_menu_label = create_text_label_with_params(
                string.format("%s_url", route.route),
                route.required_parts
              )
              local url_insert_text = create_snippet_label_with_params(
                string.format("%s_url", route.route),
                route.required_parts
              )
              table.insert(items, {
              ...
              local path_menu_label = create_text_label_with_params(
                string.format("%s_path", route.route),
                route.required_parts
              )
              local path_insert_text = create_snippet_label_with_params(
                string.format("%s_path", route.route),
                route.required_parts
              )
              table.insert(items, {
...
source.is_available = function()
  return vim.bo.filetype == "eruby"
      or vim.bo.filetype == "slim"
      or vim.bo.filetype == "haml"
      or vim.bo.filetype == "ruby"
end
